...
export class ProfileRepository {
  ...
  async getAll(): Promise<IProfile[]> {
    const res = await axios.get(`${this.uri}/profile`, {
  ...
  async getById(id: number): Promise<IProfile> {
    const res = await axios.get(`${this.uri}/profile/${id}`, {
      headers: getAuthHeaders(),
    });
    return res.data;
  }
  ...
  async create(profile: IProfileDTO): Promise<IProfile> {
    const res = await axios.post(`${this.uri}/profile`, profile, {
      headers: getAuthHeaders(),
    });
    return res.data;
  }
  ...
  async update(id: number, profile: IProfileDTO): Promise<IProfile> {
    const res = await axios.put(`${this.uri}/profile/${id}`, profile, {
      headers: getAuthHeaders(),
    });
    return res.data;
  }
  ...
  async delete(id: number): Promise<void> {
    await axios.delete(`${this.uri}/profile/${id}`, {
      headers: getAuthHeaders(),
    });
  }
}
